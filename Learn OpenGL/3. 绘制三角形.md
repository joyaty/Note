...
*每个顶点属性从被顶点缓冲对象（VBO）管理的内存上获取数据，而具体从哪个顶点缓冲对象（可能有多的顶点缓冲对象）获取数据则取决于调用**glVertexAttribPointer**被绑定在GL_ARRAY_BUFFER目标上。在调用**glVertexAttrbPointer**前，先前定义的VBO已经被绑定好了，因此顶点属性0已经链接到了顶点数据中。*
